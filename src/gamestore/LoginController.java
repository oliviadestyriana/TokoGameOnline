...
public class LoginController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        loginPasswordField.setPromptText("Kata Sandi");
    }    
    
    public void processLogin(ActionEvent event) throws Exception {
        if(application == null) {
            errorMessage.setText("Selamat Datang, " + loginUserName.getText());
        } else {
            if(!application.userLogging(loginUserName.getText(), loginPasswordField.getText())) {
                errorMessage.setText("Username/Kata Sandi anda SALAH!");
            }
        }
    }
